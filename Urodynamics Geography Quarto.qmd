...
load_cached_or_fetch_female_population <- function(geography, cache_path = NULL) {
  ...
  if (identical(geography, "tract")) {
    female_pop <- rbindlist(lapply(c(state.abb, "DC", "PR"), function(state_code) {
      get_acs(
        geography = "tract",
        state = state_code,
...
points_sf <- usmap_transform(data = geo_codes_arc,
...
states_n <- states_n %>%
  left_join(data.frame(state_full = c(state.name, "District of Columbia", "Puerto Rico"),
                       state_abb = c(state.abb, "DC", "PR")),
                   by = c("state" = "state_abb"))

#Get state female population size
...
uro_matrix <- matrix(ncol = 2, nrow = nrow(geo_codes_arc),
       c(geo_codes_arc$long,
         geo_codes_arc$lat)
         )
...
#Convert to miles
min_distances <- (min_distances * miles_per_meter)
...
#Import census tract female population file
ct_data <- load_cached_or_fetch_female_population(
  "tract",
  "ACS 2023 5-year Female Tract Population.csv"
) %>%
  rename(pop = estimate)
...
#Calculated weighted median distance for women
gdata[, wtd.quantile(dist, pop, probs = c(0.25, 0.5, 0.75))
      %>% round(1)]

##N and % of women more than 100 miles from a urodynamics testing center
